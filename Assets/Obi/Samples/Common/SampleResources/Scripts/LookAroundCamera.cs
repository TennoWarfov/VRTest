...
namespace Obi.Samples
{
    ...
    public class LookAroundCamera : MonoBehaviour
    {
        ...
        {
            currentShot.up = up;
            currentShot.rotation = Quaternion.LookRotation(
                position - currentShot.position,
                currentShot.up
            );
        }
        ...
        private void UpdateShot()
        {
            transform.position = Vector3.Lerp(
                ...
                translationResponse * Time.deltaTime
            );
            transform.rotation = Quaternion.Slerp(
                transform.rotation,
                currentShot.rotation,
                rotationResponse * Time.deltaTime
            );
            cam.fieldOfView = Mathf.Lerp(
                cam.fieldOfView,
                currentShot.fieldOfView,
                fovResponse * Time.deltaTime
            );
        }
        ...
        private void LateUpdate()
        {
            ...
            if (Mouse.current.leftButton.isPressed && !focusUI)
            {
                float deltaX = Mouse.current.delta.x.ReadValue() * rotationSpeed;
                float deltaY = Mouse.current.delta.y.ReadValue() * rotationSpeed;
                Quaternion fwd =
                    currentShot.rotation
                    * Quaternion.AngleAxis(deltaX, Vector3.up)
                    * Quaternion.AngleAxis(deltaY, -Vector3.right);
